...
bool ImGui::Checkbox ( const char* label, bool* v )
{
    ...
    it_anim->second.active_rect_anim = ImLerp ( it_anim->second.active_rect_anim, *v ? 1.0f : ( hovered ? 0.2f : 0.0f ), 0.08f * ( 1.0f - ImGui::GetIO ( ).DeltaTime ) );
    it_anim->second.outline_anim = ImLerp ( it_anim->second.outline_anim, *v ? 0.1f : ( hovered ? 0.1f : 0.0f ), 0.04f * ( 1.0f - ImGui::GetIO ( ).DeltaTime ) );

    window->DrawList->AddRectFilled ( total_bb.Min, total_bb.Min + ImVec2 ( square_sz, square_sz ), ImColor ( 16, 12, 29 ), 2.0f );
    window->DrawList->AddRectFilled ( total_bb.Min, total_bb.Min + ImVec2 ( square_sz, square_sz ), ImColor ( 104 / 255.0f, 105 / 255.0f, 255 / 255.0f, it_anim->second.active_rect_anim ), 2.0f );
    RenderCheckMark ( window->DrawList, ImVec2 ( total_bb.Min.x + 3, ( total_bb.Min.y + total_bb.Max.y ) / 2 - 4 ), ImColor ( 1.0f, 1.0f, 1.0f, it_anim->second.active_rect_anim ), 9.0f );

    window->DrawList->AddRect ( total_bb.Min, total_bb.Min + ImVec2 ( square_sz, square_sz ), ImColor ( 1.0f, 1.0f, 1.0f, 0.1f + it_anim->second.outline_anim ), 2.0f );
    window->DrawList->AddText ( ImVec2 ( total_bb.Max.x - label_size.x - 5, total_bb.Min.y ), ImColor ( 1.0f, 1.0f, 1.0f ), label );
...
bool ImGui::Checkbox2 ( const char* label, bool* v )
{
    ...
    ImU32 check_col = ImColor ( 0.66f, 0.27f, 0.46f, 0.2f * text_selected->second * 6.f );

    window->DrawList->AddRectFilled ( check_bb.Min + ImVec2 ( 1, 1 ), check_bb.Max - ImVec2 ( 1, 2 ), ImColor ( 25, 25, 25, 255 ), 2.f );
    window->DrawList->AddRect ( check_bb.Min + ImVec2 ( 1, 1 ), check_bb.Max - ImVec2 ( 1, 2 ), ImColor ( 10, 10, 10 ), 2.f );
    window->DrawList->AddRectFilled ( check_bb.Min + ImVec2 ( 1, 1 ), check_bb.Max - ImVec2 ( 1, 2 ), check_col, 2.f );

    if ( *v == true )
        window->DrawList->AddRectFilled ( ImVec2 ( check_bb.Min.x, check_bb.Max.y - 2 ), check_bb.Max, ImColor ( 110, 45, 214 ) );

    ImGui::PushStyleColor ( ImGuiCol_Text, ImVec4 ( 0.21f, 0.21f, 0.21f, 1.f ) );
    if ( label_size.x > 0.0f )
    ...
    ImGui::PushStyleColor ( ImGuiCol_Text, ImVec4 ( 1.f, 1.f, 1.f, 0.4f * text_selected->second * 2 ) );
    if ( label_size.x > 0.0f )
        RenderText ( ImVec2 ( check_bb.Max.x + style.ItemInnerSpacing.x + 1, check_bb.Min.y + ( ( check_bb.Max.y - check_bb.Min.y ) / 2 ) - label_size.y / 2 - 1 ), label );
    ImGui::PopStyleColor ( 1 );
...
bool ImGui::Tab ( const char* label, bool selected )
{
    ...
    draw->AddRectFilled ( ImVec2 ( bb.Min.x, bb.Max.y - 2 ), bb.Max, ImColor ( 110, 45, 214, a ) );
...
bool ImGui::BeginChildExss ( const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags )
{
    ...
    window->DrawList->AddRect ( bb.Min, bb.Max, ImColor ( 110, 45, 214 ), 0, 15, 1 );
    window->DrawList->AddRect ( bb.Min, bb.Max, ImColor ( 110, 45, 214 ), 0, 15, 1 );
...
bool ImGui::SliderScalar ( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags )
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID ( label );
    const float w = GetWindowWidth ( ) - 50;

    const ImVec2 label_size = CalcTextSize ( label, NULL, true );
    const ImRect total_bb ( window->DC.CursorPos + ImVec2 ( 21, 0 ), window->DC.CursorPos + ImVec2 ( w, label_size.y + 14 ) );
    ...
    window->DrawList->AddRectFilled ( frame_bb.Min, frame_bb.Max, ImColor ( 16, 12, 29 ), 2.0f );
    window->DrawList->AddRectFilled ( frame_bb.Min, ImVec2 ( grab_bb.Min.x + 3, frame_bb.Max.y ), ImColor ( 104 / 255.0f, 105 / 255.0f, 255 / 255.0f ), 2.0f );
    window->DrawList->AddRect ( frame_bb.Min, frame_bb.Max, ImColor ( 1.0f, 1.0f, 1.0f, 0.05f + it_anim->second.outline_anim ), 2.0f );

    RenderText ( total_bb.Min, label );
    RenderText ( ImVec2 ( grab_bb.Min.x - CalcTextSize ( value_buf ).x / 2, frame_bb.Min.y + 3 ), value_buf );
